...
def gridtopology(caller: str = "user",
                 input_dir: str = "",
                 output_dir: str = "",
...
def main():
    ...
    # Invoke gridtopology and capture err_msg
    err_msg = gridtopology(
        caller=caller,
        input_dir=input_dir,
        output_dir=output_dir,
        save_topology=False,
        display_topology=False,
        graph_title="Test Run",
        marker_size=1.0,
        timestamp=False,
        verbose=verbose
    )
    ...
    # Check return value
    if err_msg:
        print(f"gridtopology returned error: {err_msg}")
        sys.exit(1)
    else:
        print("gridtopology completed successfully with no error.")

    print("\n--- Inspect state ---")
    print("working_buses:", ST.state.working_buses.shape)
    print(ST.state.working_buses.head(10).to_string(index=False))

    print("\nworking_segments:", ST.state.working_segments.shape)
    print(ST.state.working_segments.head(10).to_string(index=False))

    am = ST.state.adj_mat
    print("\nadj_mat shape:", am.shape, "edges:", int(am.sum()))
    # 행렬 전부는 길게 나오니, 처음 10개 간선만 확인
    print("first 10 edges (i,j):", np.argwhere(am)[:10])
