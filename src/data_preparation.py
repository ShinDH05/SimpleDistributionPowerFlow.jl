...
def working_lines():
    ...
    while len(unnumbered) > 0:
        for i in range(len(wb)):
            if wb.at[i, 'number'] == 0:
                cur_id = wb.at[i, 'id']  # current id
                parent_seg = ws[ws['bus2'] == cur_id]
                precedent = parent_seg.iloc[0]['bus1']
                parent_num = wb.loc[wb['id'] == precedent, 'number'].iloc[0]
                if parent_num > 0:
                    wb.at[i, 'number'] = int(parent_num) + 1
        unnumbered = wb[(wb['type'] == 3) & (wb['number'] == 0)]
...
def data_preparation():
    ...
    for idx in lines.index:
        ...
        if seg_type == 1:
            ...
            Zaa = lines.at[idx, 'Zaa'];
            Zbb = lines.at[idx, 'Zbb'];
            Zcc = lines.at[idx, 'Zcc']
            phase_str = ""
            ...
            lines.at[idx, 'Zab'] = 0 + 0j;
            lines.at[idx, 'Zac'] = 0 + 0j;
            lines.at[idx, 'Zbc'] = 0 + 0j
            lines.at[idx, 'Baa'] = 0 + 0j;
            lines.at[idx, 'Bab'] = 0 + 0j;
            lines.at[idx, 'Bac'] = 0 + 0j
            lines.at[idx, 'Bbb'] = 0 + 0j;
            lines.at[idx, 'Bbc'] = 0 + 0j;
            lines.at[idx, 'Bcc'] = 0 + 0j
    ...
    gen_cols = ['bus1', 'bus2'] + [f"{m}_{p}_{q}" for m in ['a', 'b', 'c', 'd', 'A', 'B'] for p in range(1, 4) for q in
                                   range(1, 4)]
    gen_lines_list = []
    ...
    for _, line in lines.iterrows():
        ...
        if m_type == 1 or m_type == 3:
            ...
            ch = tr.iloc[0]['conn_high'];
            cl = tr.iloc[0]['conn_low'];
            Zt = tr.iloc[0]['Zt']
