    ...
    while max_diff > tolerance:
        ...
        if ST.state.has_distributed_gen:
            ...
            if getattr(ST.state, "has_pqv_distributed_gen", False):
                wb = ST.state.working_buses.copy()        # DataFrame of current bus voltages
                pqv_df = ST.state.pqv_distributed_gen.copy()
                if pqv_df is not None and not pqv_df.empty:
                    ...
                    for idx, dg in pqv_df.iterrows():
                        ...
                        v_set = dg['kv_set'] * 1000 / (3 ** 0.5)   # line-to-neutral setpoint voltage (선간전압[kV]으로 가정하고 v_set에서 상전압 변환(÷√3))
                        xd = dg['xd']
                        ...
                        for col in ['k_1', 'k_2', 'k_3']:
                            if col in ST.state.loads.columns:
                                new_load[col] = None
                        ST.state.loads = pd.concat([ST.state.loads, pd.DataFrame([new_load])], ignore_index=True).copy()
            ...
            if getattr(ST.state, "has_pi_distributed_gen", False):
                ...
                pi_df = ST.state.pi_distributed_gen.copy()
